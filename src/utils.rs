...
pub(crate) fn vec_i64_into_vec_u64(mut vec: Vec<i64>) -> Vec<u64> {
    let p = vec.as_mut_ptr();
    let len = vec.len();
    let cap = vec.capacity();
    std::mem::forget(vec);
...
pub(crate) fn tightly_packed<T>(unpacked_array: &[T], bits_per_value: usize) -> Vec<u64>
where
    u64: From<T>,
    T: Copy,
{
    ...
    for long_index in 0..packed_len {
        ...
        let high_unpacked_index = std::cmp::min(
            unpacked_len,
            low_unpacked_index + values_overlapping_u64_max,
        );
        ...
        for unpacked_index in low_unpacked_index..high_unpacked_index {
            ...
            if value_bit_index < packed_long_bit_index {
                long |= value
                    .checked_shr((packed_long_bit_index - value_bit_index) as u32)
                    .unwrap_or(0);
            } else {
                long |= value
                    .checked_shl((value_bit_index - packed_long_bit_index) as u32)
                    .unwrap_or(0);
            }
        }
...
pub(crate) fn paddedly_packed<T>(unpacked_array: &[T], bits_per_value: usize) -> Vec<u64>
where
    u64: From<T>,
    T: Copy,
{
    ...
    for long_index in 0..packed_len {
        ...
        for internal_index in 0..full_values_per_u64 {
            ...
            long |= (Into::<u64>::into(unpacked_array[unpacked_index]) & value_mask)
                << (internal_index * bits_per_value);
        }
        packed.push(long);
    }
    packed
...
mod tests {
    ...
    const PADDEDLY_PACKED_5: [u64; 3] =
        [0x0020863148418841, 0x01018A7260F68C87, 0x000000000000008B];
    ...
    fn test_tight_unpacking() {
        assert_eq!(
            UNPACKED_U8,
            tightly_unpacked(&TIGHTLY_PACKED_5, 5).as_slice()[..26]
        );
    }
    ...
    fn test_padded_packing() {
        assert_eq!(
            PADDEDLY_PACKED_5,
            paddedly_packed(&UNPACKED_U8, 5).as_slice()
        );
    }
    ...
    fn test_padded_unpacking() {
        assert_eq!(
            UNPACKED_U8,
            paddedly_unpacked(&PADDEDLY_PACKED_5, 5).as_slice()[..26]
        );
    }
